...
export function useMindMap() {
  const getPosition = (ref: React.RefObject<HTMLElement>): NodePosition => {
    if (!ref || !ref?.current) return undefinedPosition

    const { right, bottom, left, top } = ref?.current.getBoundingClientRect()
